...
const Button = ({ label, btnType = "button", btnBase, className, loading }) => {
    let btnClass = '';
    switch (btnBase) {
        case 'primary':
    ...
    return (
        <button type={btnType} className={'px-2 py-1 tracking-wide transition-colors duration-200 transform rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 ' + btnClass + ' ' + className}>
            {loading && <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white font-semibold" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
